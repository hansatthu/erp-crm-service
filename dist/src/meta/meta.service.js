...
let MetaService = MetaService_1 = class MetaService {
    ...
    async getUserProfile(psid, pageId) {
    ...
    async addLabelToUser(psid, labelName, pageId) {
        const accessToken = this.getPageAccessToken(pageId);
        if (!accessToken)
            return false;
        try {
    ...
    async sendMessage(recipientId, text, pageId) {
        const accessToken = this.getPageAccessToken(pageId);
        if (!accessToken) {
    ...
    async sendOrderReceipt(recipientId, orderData, pageId) {
        const accessToken = this.getPageAccessToken(pageId);
        if (!accessToken) {
    ...
    async sendAction(recipientId, action, pageId) {
        const accessToken = this.getPageAccessToken(pageId);
        if (!accessToken)
            return null;
        try {
